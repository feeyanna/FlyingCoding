...
# config目录: 配置文件目录
# logs目录: 项目运行日志目录
# logs/micro-business-A-api_startup.log: 记录启动日志
...
# 项目名称
APPLICATION="micro-business-A-api-1.0.0-SNAPSHOT"

# 项目日志名称
APPLICATION_LOG="micro-business-A-api"
...
# 项目启动日志输出绝对路径
LOG_STARTUP_PATH="${LOG_DIR}/${APPLICATION_LOG}_startup.log"
...
# 如果项目运行日志存在,则重命名备份
if [ -f "${LOG_PATH}" ]; then
	mv ${LOG_PATH} "${LOG_BACK_DIR}/${APPLICATION_LOG}_back_${NOW}.log"
fi
...
#==========================================================================================
JAVA_OPT="-server -Xms8g -Xmx8g -Xmn4g -XX:MetaspaceSize=1g -XX:MaxMetaspaceSize=2g"
